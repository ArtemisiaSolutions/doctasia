...
var appDbProvider = new Provider(confMgr.getConfig("appDatabase.name"),
    confMgr.getConfig("appDatabase.host"),
    confMgr.getConfig("appDatabase.port"),
    confMgr.getConfig("appDatabase.user"),
    confMgr.getConfig("appDatabase.password"),
    function() {

        var runner = new ParallelRunner()

        runner.add(appDbProvider.loadCollection, 'users')

        runner.add(appDbProvider.loadCollection, 'repositories')

        runner.add(appDbProvider.loadCollection, 'buildjobs')

        runner.add(appDbProvider.loadCollection, 'work_queue')

        runner.add(appDbProvider.loadCollection, 'work_queue_archive')

        runner.add(appDbProvider.loadCollection, 'commit')

        runner.add(appDbProvider.loadCollection, 'ssh')

        runner.add(appDbProvider.loadCollection, 'worker', [
            { index: {'host': 1}, options: {unique: true} }
        ])

        runner.run(function() {

            logger.debug("App db is ready, emitting event")
            module.exports.AppEvent.emit('ready')

        })

    })

var adminDbProvider = new Provider(confMgr.getConfig("adminDatabase.name"),
    confMgr.getConfig("adminDatabase.host"),
    confMgr.getConfig("adminDatabase.port"),
    confMgr.getConfig("adminDatabase.user"),
    confMgr.getConfig("adminDatabase.password"),
    function() {

        var runner = new ParallelRunner()

        runner.add(adminDbProvider.loadCollection, 'admin')

        runner.run(function() {

            logger.debug("Admin db is ready, emitting event")
            module.exports.AdminEvent.emit('ready')
